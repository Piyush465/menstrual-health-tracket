#include <iostream>
#include <vector>      // DATA STRUCTURE 1: Vectors for symptoms, yoga, breathing, history
#include <queue>       // DATA STRUCTURE 2: Priority Queue for food recommendations
#include <stack>       // DATA STRUCTURE 3: Stack for undo/redo functionality
#include <map>         // DATA STRUCTURE 4: Map for cycle phase transitions (graph)
#include <unordered_map> // DATA STRUCTURE 5: Hash table for fast food lookup
#include <set>         // DATA STRUCTURE 6: Set for unique categories
#include <string>
#include <algorithm>
#include <ctime>
#include <sstream>
...
struct HealthEntry {
    ...
    string phase;
    int dayOfCycle;
    vector<int> symptoms;           // DATA STRUCTURE 1: Vector stores symptom IDs
    vector<int> moods;              // DATA STRUCTURE 1: Vector stores mood IDs
    int energyLevel;
    string notes;
    string condition;
...
// DATA STRUCTURE 7: Array for O(1) phase lookup by day
...
// DATA STRUCTURE 4: Map (ordered) for cycle phase graph transitions
...
// DATA STRUCTURE 7: Array for symptom names (20 symptoms)
...
// DATA STRUCTURE 7: Array for mood names (15 moods)
string moodMenu[15] = {
...
// DATA STRUCTURE 6: Set to track unique categories
...
// DATA STRUCTURE 5: Unordered Map (Hash Table) for O(1) food lookup by condition
...
// DATA STRUCTURE 1: Vector stores all yoga asanas with complete details
...
void printLine(char ch = '=', int length = 80) { cout << string(length, ch) << "\n"; }
...
string formatDuration(int minutes) {
    if (minutes < 60) return to_string(minutes) + " minutes";
...
void displayHistory(int count) {
    ...
    cout << left;
    cout << "  " << setw(15) << "Date" << setw(15) << "Phase" << setw(22) << "Condition" << setw(10) << "Energy" << "\n";
    printLine('-');
    for (int i = start; i <= currentHistoryIndex; i++) {
        cout << "  " << setw(14) << historyStack[i].date << setw(14) << historyStack[i].phase
             << setw(21) << historyStack[i].condition << setw(9) << (to_string(historyStack[i].energyLevel) + "/10") << "\n";
        ...
        if (!historyStack[i].notes.empty()) cout << "     Notes: " << historyStack[i].notes << "\n";
        cout << "\n";
    }
    printLine();
}
...
void saveToHistory(HealthEntry& entry) {
    if (currentHistoryIndex + 1 < (int)historyStack.size()) historyStack.resize(currentHistoryIndex + 1);
...
void displayCycleInfo() {
    ...
    for (const auto& transition : cycleGraph) cout << "    " << transition.first << " → " << transition.second << "\n";
    cout << "\n  Phase Duration:\n";
...
int main() {
    ...
    while (true) {
        ...
        if (command == "exit") {
            ...
            cout << "  Day of cycle (1-28): "; cin >> entry.dayOfCycle;
            if (entry.dayOfCycle < 1 || entry.dayOfCycle > 28) {
                cout << "\n  ❌ Invalid! Please enter day between 1-28.\n"; continue;
            }
            cout << "  Energy level (1-10): "; cin >> entry.energyLevel;
            displaySymptomMenu();
            int symptomCount; cout << "  How many symptoms do you have? "; cin >> symptomCount;
            for (int i = 0; i < symptomCount; i++) {
                int symptomId; cout << "  Enter symptom ID " << (i+1) << ": "; cin >> symptomId;
                if (symptomId >= 0 && symptomId < 20) entry.symptoms.push_back(symptomId);
                else { cout << "  ⚠️  Invalid ID! Must be 0-19. Try again.\n"; i--; }
            }
            displayMoodMenu();
            int moodCount; cout << "  How many moods are you experiencing? "; cin >> moodCount;
            for (int i = 0; i < moodCount; i++) {
                int moodId; cout << "  Enter mood ID " << (i+1) << ": "; cin >> moodId;
                if (moodId >= 0 && moodId < 15) entry.moods.push_back(moodId);
                else { cout << "  ⚠️  Invalid ID! Must be 0-14. Try again.\n"; i--; }
            }
            ...
            stringstream ss;
            ss << (1900 + timeInfo->tm_year) << "-" << setfill('0') << setw(2) << (1 + timeInfo->tm_mon) << "-" << setw(2) << timeInfo->tm_mday;
            entry.date = ss.str();
            cin.ignore();
            cout << "\n  Additional notes (optional): "; getline(cin, entry.notes);
            saveToHistory(entry);
            printLine();
            ...
            string condition; int veg, vegan;
            cout << "\n  Available conditions:\n";
            cout << "  - Normal\n  - PMS\n  - Fatigue / Low Energy\n  - Menstrual Discomfort\n";
            cout << "  - Heavy Flow\n  - Bloating\n  - Anxious\n  - Mood Swings\n";
            cout << "  - Breakouts\n  - Headaches\n\n";
            cout << "  Enter condition: "; cin.ignore(); getline(cin, condition);
            cout << "  Vegetarian? (1=yes, 0=no): "; cin >> veg;
            cout << "  Vegan? (1=yes, 0=no): "; cin >> vegan;
            cout << "\n"; displayFoodRecommendations(condition, veg == 1, vegan == 1);
        }
        else if (command == "yoga") {
            string phase, difficulty;
            cout << "\n  Phase (Menstrual/Follicular/Ovulation/Luteal/All): "; cin >> phase;
            cout << "  Difficulty (beginner/intermediate/advanced/all): "; cin >> difficulty;
            cout << "\n"; displayYogaRecommendations(phase, difficulty);
        }
        else if (command == "breathing") {
            string purpose;
            cout << "\n  Purpose (anxiety/stress/sleep/energy/all): "; cin >> purpose;
            cout << "\n"; displayBreathingExercises(purpose);
        }
        else if (command == "history") {
            int count; cout << "\n  Number of entries to show: "; cin >> count;
